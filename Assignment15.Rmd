...
num_hawks <- hawks_total %>% nrow()
num_train <- floor(num_hawks*0.6)
num_test <- num_hawks - num_train

set.seed(123)
test_inds <- sample(seq(num_hawks), num_test)
train_inds <- setdiff(seq(num_hawks), test_inds)
...
lda_train_predicted_y_broken <- broken_classifier(hawks_train_x)
lda_train_error_broken <- mean(abs(lda_train_predicted_y_broken-hawks_train_y))
lda_train_error_broken

lda_test_predicted_y_broken <- broken_classifier(hawks_test_x)
lda_test_error_broken <- mean(abs(lda_test_predicted_y_broken-hawks_test_y))
...
lda_train_predicted_y <- predict(lda_model,hawks_train_x)$class %>% as.character() %>% as.numeric()
lda_train_error <- mean(abs(lda_train_predicted_y-hawks_train_y))
lda_train_error

lda_test_predicted_y <- predict(lda_model,hawks_test_x)$class %>% as.character() %>% as.numeric()
...
logist_plot + stat_function(fun=logist_func) + xlab("z") + ylab("S(z)")
